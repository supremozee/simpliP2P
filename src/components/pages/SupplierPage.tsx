...
const SupplierPage = () => {
  ...
  const renderRow = (supplier: FilteredSupplier, index: number) => (
    <TableRow
      key={supplier.id}
      data={[
        <div key={`select-${supplier.id}`} className="flex items-center justify-center">
          <button 
            onClick={() => toggleSelectItem(supplier.id)}
            className="flex items-center justify-center w-5 h-5 focus:outline-none"
            aria-label={isSelected(supplier.id) ? "Deselect supplier" : "Select supplier"}
          >
      ...
      className={isSelected(supplier.id) ? 'bg-blue-50 hover:bg-blue-100' : index % 2 === 0 ? 'bg-white hover:bg-gray-50' : 'bg-gray-50 hover:bg-gray-100'}
      index={index}
    />
  );
