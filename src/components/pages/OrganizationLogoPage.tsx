...
const OrganizationLogoPage = () => {
  const { uploadOrganizationLogo, loading, errorMessage } = useUploadOrganizationLogo();
  const {currentOrg} = useStore()
  const handleFileChange = async(event:React.ChangeEvent<HTMLInputElement>)=> {
if(event.target.files && event.target.files[0]) {
  await uploadOrganizationLogo(event.target.files[0], currentOrg)
}
  }
  return (
    <div className='flex h-screen w-full font-roboto'>
      ...
      <div className='flex justify-center flex-col gap-4 w-full sm:w-1/2 p-5 sm:p-[150px]'>
        <div className='flex flex-col items-center justify-center'>
          <strong className='text-[24px] text-center sm:text-[30px] font-[700]'>Upload your organization logo</strong>
