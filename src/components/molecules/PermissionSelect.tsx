...
const PERMISSIONS: { value: Permission; label: string; description: string; parent?: string }[] = [
    {
      value: "manage_users",
      label: "Manage Users",
...
const PermissionSelect: React.FC<PermissionSelectProps> = ({ value, onChange, error }) => {
  const options: MultiSelectOption<Permission>[] = PERMISSIONS.reduce((acc, permission) => {
    if (!permission.parent) {
      const option: MultiSelectOption<Permission> = {
        ...
        options: PERMISSIONS
          .filter(p => p.parent === permission.value)
          .map(child => ({
            label: child.label,
            value: child.value,
  ...
  const selectedOptions: MultiSelectOption<Permission>[] = PERMISSIONS
  ...
  const CustomOption: React.FC<{ data: MultiSelectOption<Permission>; isSelected: boolean }> = ({ data, isSelected }) => {
    ...
    return (
      <div 
        className={`
          py-2 px-3 
          ${isParent ? 'bg-blue-50 border-b border-blue-100' : 'bg-white hover:bg-gray-50'} 
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {isSelected && (
            <span className={isParent ? "text-blue-600" : "text-gray-600"}>✓</span>
          )}
        </div>
        {data.description && (
          <p className={`
