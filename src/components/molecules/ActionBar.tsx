...
const ActionBar: React.FC<ActionBarTypes> = ({
  ...
  const activeFilterCount =
    Object.keys(activeFilters).length + (startDate || endDate ? 1 : 0);
          ...
          <div className="flex items-center gap-2">
                ...
                {isFilterOpen && (
                  <div className="absolute right-0 mt-1 w-56 bg-white rounded-lg shadow-md py-2 z-50 filter-menu border border-gray-200">
                    <div className="px-3 py-2 border-b border-gray-100 flex justify-between items-center">
                      <h3 className="text-sm font-medium #181819">
                        Filter {type}
                      </h3>
                      {activeFilterCount > 0 && (
                        <button
                    ...
                    {filterOptions.map((filter) => (
                      <div
                        key={filter.value}
                        className="relative px-3 py-2 hover:bg-gray-50"
                      >
                        <button
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <span className="font-medium #181819">
                              {filter.label}
                            </span>
                            {activeFilters[filter.value] && (
                              <span className="text-xs px-1.5 py-0.5 bg-primary/10 text-primary rounded">
                                {filter.options.find(
                                  (o) => o.value === activeFilters[filter.value]
                                )?.label || activeFilters[filter.value]}
                              </span>
                            )}
                          </div>
                          <BsChevronDown
                            className={`w-3 h-3 text-gray-400 transition-transform ${
                              activeFilter === filter.value ? "rotate-180" : ""
                            }`}
                          />
                        </button>
                          ...
                          <div className="absolute left-20 top-0 w-56 bg-white shadow-lg rounded-lg py-2 -ml-2 z-50 border border-gray-200">
                            ...
                            <div className="max-h-60 overflow-y-auto py-1">
                              {filter.options.map((option) => (
                                <button
                                  key={option.value}
                                  className={`w-full px-3 py-2 text-left text-xs hover:bg-gray-50 flex justify-between items-center ${
                                  ...
                                  onClick={() => {
                                    if (onFilter)
                                      onFilter(filter.value, option.value);
                                    setIsFilterOpen(false);
                                    setActiveFilter(null);
                                  }}
                                >
            ...
            {showDate && (
              <div className="relative">
                <Tooltip
                  content={
                    startDate || endDate ? "Edit Date Range" : "Date Range"
                  }
                >
                  <Button
                    kind="white"
                  ...
                  <div className="absolute right-0 mt-1 w-72 bg-white rounded-lg shadow-md p-4 z-50 date-picker border border-gray-200">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        kind="white"
                        width="full"
                        className="#181819 text-xs flex justify-center items-center border border-gray-300"
                      >
                        Cancel
                      </Button>
                      <Button
                ...
                {isExportMenuOpen && (
                  <div className="absolute right-0 mt-1 w-48 bg-white rounded-lg shadow-md py-2 z-50 export-menu border border-gray-200">
                    ...
                    {exportFormats.map((fmt) => (
                      ...
                      >
                        <span className="#181819">{fmt.label} format</span>
                        {format === fmt.value && (
        ...
        {activeFilterCount > 0 && (
          <div className="px-3 py-2 flex items-center flex-wrap gap-2 mt-2">
            {Object.entries(activeFilters).map(([filterType, value]) => {
              ...
              const optionLabel =
                filterOption?.options.find((o) => o.value === value)?.label ||
                value;
            ...
            <button
              className="text-xs text-gray-500 hover:#181819 underline ml-1"
              onClick={handleClearAllFilters}
            >
              Clear all filters
            </button>
        ...
        {isAdvancedFilterOpen && (
          <div className="mt-2 p-4 bg-white rounded-lg border border-gray-200 shadow-sm advanced-filter-menu">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-medium #181819">Advanced Filters</h3>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filterOptions.map((filter) => (
                <div key={filter.value} className="mb-3">
                  <label className="block text-xs font-medium #181819 mb-1">
                    {filter.label}
                  </label>
                  <select
                    className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary"
