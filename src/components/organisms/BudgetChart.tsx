...
const BudgetChart: React.FC<BudgetChartProps> = ({ budgetData }) => {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload?.length) {
      ...
      const percentage =
        parsedBudget.allocated > 0
          ? ((data.value / parsedBudget.allocated) * 100).toFixed(1)
          : "0.0";

      return (
        <div className="bg-white border rounded-md shadow-md p-3">
          <p className="text-sm font-medium #181819">{data.name}</p>
          <p className="text-sm mt-1">
            <span className="font-medium">
              {parsedBudget.currency} {format_price(data.value)}
            </span>
          ...
          <div className="mt-2 grid grid-cols-3 gap-4">
            <div className="flex items-center">
              ...
              <span className="text-sm text-gray-600">
                Available:{" "}
                <span className="font-medium">
                  {format_price(parsedBudget.balance)}
                </span>
              </span>
            </div>
            <div className="flex items-center">
              ...
              <span className="text-sm text-gray-600">
                Reserved:{" "}
                <span className="font-medium">
                  {format_price(parsedBudget.reserved)}
                </span>
              </span>
            </div>
            <div className="flex items-center">
              ...
              <span className="text-sm text-gray-600">
                Used:{" "}
                <span className="font-medium">
                  {format_price(parsedBudget.used)}
                </span>
              </span>
            </div>
          </div>
