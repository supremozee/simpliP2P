...
import { IoClose, IoDocumentTextSharp, IoCheckmarkCircle, IoWarning } from "react-icons/io5";
import { FaArrowLeft, FaArrowRight, FaFileUpload, FaUserTie, FaBuilding, FaCalendarAlt } from "react-icons/fa";
...
const CreatePurchaseOrder = () => {
  const { register, handleSubmit, formState: { errors }, setValue, watch, reset } = useForm<PurchaseOrderFormData>({
  ...
  const { isOpen, setIsOpen, currentOrg, setPr, orgName, upload, setUpload } = useStore();
  const { uploadFile, loading: isUploading} = useFileManager();
  const { data: requisitions, isLoading: isRequisitionsLoading, isError: isRequisitionsError } = 
    useFetchPurchaseRequisition(currentOrg, "APPROVED");
  const { data: suppliersData, isLoading: isSuppliersLoading } = useFetchSuppliers(currentOrg);
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!Object.keys(ACCEPTED_FILE_TYPES).includes(file.type)) {
      setFileError("Invalid file type. Please upload jpg, png, pdf, excel, or csv files.");
      return;
    }
  ...
  const filteredRequisitions = useMemo(() => {
    if (!requisitions?.data?.requisitions || !ordersData?.data?.orders) return [];
    
    return requisitions.data.requisitions.filter((requisition) => {
      return !ordersData.data.orders.some(
        (order) => order.purchase_requisition.pr_number === requisition.pr_number
      );
    });
  ...
  const isLoading = isRequisitionsLoading || isOrdersLoading || isSuppliersLoading;
            ...
            filteredRequisitions.length > 0 ? (
                  ...
                  <div className="flex justify-between items-center p-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">PR-{filteredRequisitions[currentIndex]?.pr_number}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <Button
                        ...
                        disabled={currentIndex === filteredRequisitions.length - 1}
                        className={cn(
                          "p-2 rounded-lg transition-colors",
                ...
                <div className="grid grid-cols-2 gap-8 p-6">
                  ...
                  <div className="space-y-6">
                    ...
                    >
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <h3 className="text-lg font-semibold text-gray-800">Requestor Details</h3>
                      </div>
                      <div className="grid gap-4">
                        <div>
                          <p className="text-sm text-gray-500">Department & Branch</p>
                          <div className="flex items-center gap-2 mt-1">
                            <FaBuilding className="w-4 h-4 text-gray-400" />
                            <p className="text-sm font-medium text-gray-700">
                              {filteredRequisitions[currentIndex]?.department?.name}, {filteredRequisitions[currentIndex]?.branch?.name}
                            </p>
                          </div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div className="flex items-center justify-between mt-1">
                            <div className="flex items-center gap-2">
                              ...
                              <p className="text-sm font-medium text-gray-700">
                                {new Date(filteredRequisitions[currentIndex]?.needed_by_date).toLocaleDateString()}
                              </p>
                            </div>
                            <span className={cn(
                              "px-3 py-1 rounded-full text-xs font-medium",
                              calculateDaysRemaining(filteredRequisitions[currentIndex]?.needed_by_date).includes("Past")
                                ? "bg-red-100 text-red-700"
                                : "bg-green-100 text-green-700"
                            )}>
                              {calculateDaysRemaining(filteredRequisitions[currentIndex]?.needed_by_date)}
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="space-y-6">
                    ...
                    >
                      <div className="flex items-center gap-3 mb-4">
                        <IoDocumentTextSharp className="w-5 h-5 text-primary" />
                        <h3 className="text-lg font-semibold text-gray-800">Requisition Details</h3>
                      </div>
                      <div className="space-y-4">
                        <TextAreaField
                          label="Description of Goods/Services"
                          value={filteredRequisitions[currentIndex]?.request_description}
                          readOnly
                          rows={4}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <InputField
                            label="Quantity"
                            type="number"
                            value={filteredRequisitions[currentIndex]?.quantity?.toString()}
                            readOnly
                            className="bg-gray-50"
                            name="quantity"
                          ...
                          <InputField
                            ...
                            type="number"
                            value={filteredRequisitions[currentIndex]?.estimated_cost?.toString()}
                            readOnly
                            className="bg-gray-50"
                            name="estimated_cost"
                        ...
                        <TextAreaField
                          ...
                          value={filteredRequisitions[currentIndex]?.approval_justification ?? "No justification provided"}
                          readOnly
                          rows={3}
                          className="bg-gray-50"
                          name="justification"
              ...
              <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                <div className="space-y-4 mb-8">
                  ...
                  <h3 className="text-xl font-semibold text-gray-800">No Approved Requisitions</h3>
                  <p className="text-gray-500 max-w-md mx-auto">
                    There are currently no approved requisitions available to create a purchase order from.
                    You can view existing requisitions or track current orders.
                  </p>
                </div>
