...
const calculatePriority = (neededByDate: string): PriorityLevel => {
  ...
  const daysUntilNeeded = Math.ceil(
    (neededBy.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
  );
...
const PriorityBadge = ({ priority }: { priority: PriorityLevel }) => {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-xs font-medium border ${colors[priority]}`}
    >
...
const RequisitionsApprovalPage = () => {
  ...
  const {
    ...
    refetch: refetchPendingRequisitions,
  } = useFetchPurchaseRequisition(currentOrg, "PENDING");
  ...
  const [filterPriority, setFilterPriority] = useState<"all" | PriorityLevel>(
    "all"
  );
    ...
    return [...requisitions].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "amount":
          ...
          break;
        case "requestor":
          comparison = (a.requestor_name || "").localeCompare(
            b.requestor_name || ""
          );
          break;
        case "priority":
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label
                htmlFor="search"
                className="block text-sm font-medium #181819 mb-1"
              >
                Search
              </label>
                  ...
                  <div className="flex items-start justify-between relative z-10">
                    <div className="space-y-4 flex-grow pr-4">
                        ...
                        <div>
                          <div className="flex items-center gap-3">
                            <h3 className="text-lg font-medium text-primary">
                              {requisition.request_description.length > 40
                                ? requisition.request_description.substring(
                                    0,
                                    40
                                  ) + "..."
                                : requisition.request_description ||
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        ...
                        <div>
                          <p className="text-xs text-gray-500 mb-1 flex items-center gap-1">
                            <MdOutlineAttachMoney className="w-3 h-3" />{" "}
                            Estimated Cost
                          </p>
                          <div className="text-sm font-medium #181819 flex items-center gap-1">
                            <p>{requisition.currency}</p>
                            {format_price(
                              Number(requisition.estimated_cost) || 0,
