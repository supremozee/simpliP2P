...
const CustomSelect: React.FC<SelectProps> = ({
  ...
  useEffect(() => {
    if (isOpen && selectRef.current && dropdownRef.current) {
      ...
      if (selectRect.bottom + dropdownHeight > windowHeight && selectRect.top > dropdownHeight) {
        setPosition('top');
      } else {
        setPosition('bottom');
      }
  ...
  return (
    <div className={`w-full flex flex-col relative ${className}`} ref={selectRef}>
        ...
        >
          ...
          <div className='flex justify-between items-center p-2 border-b sticky top-0 bg-white z-10'>
            ...
            <div className='flex items-center gap-1 ml-2'>
              {component && <div onClick={(e) => e.stopPropagation()}>{component}</div>}
              <button
                type='button'
                onClick={(e) => {
                  e.preventDefault();
