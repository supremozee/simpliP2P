...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const findPr = data?.data?.requisitions.find(
    (pr) => pr.pr_number === pr_number
  );
  const savedPr = savedData?.data.requisitions.find(
    (pr) => pr.pr_number === pr_number
  );
  ...
  useEffect(() => {
    if (
      findPr &&
      savedPr?.status !== "SAVED_FOR_LATER" &&
      findPr.status !== "INITIALIZED"
    ) {
      setExistingPR(true);
    }
  }, [findPr, savedPr]);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-md max-w-2xl w-full p-6 md:p-8">
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6 border border-gray-100">
            <h3 className="font-medium #181819 mb-2">What can I do next?</h3>
            <ul className="text-sm text-gray-600 space-y-2">
              <li className="flex items-start">
                <span className="text-primary mr-2">•</span>
                <span>
                  View the PR details and status in your Purchase Requisitions
                  page
                </span>
              </li>
              ...
              {status === "REJECTED" && (
                <li className="flex items-start">
                  <span className="text-primary mr-2">•</span>
                  <span>
                    Review the rejection reason to understand what went wrong
                  </span>
                </li>
              )}
              <li className="flex items-start">
  ...
  return <div id="modal-initialize">{children}</div>;
};
