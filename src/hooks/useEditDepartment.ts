...
export default function useEditDepartment() {
  ...
  const { mutateAsync: EditDepartmentMutation, isPending:isUpdateDepartment } = useMutation({
    ...
    onError: (err: any) => {
      const message = err.response?.data?.message || err.message || 'An error occurred during department update. Please try again.';
      notifyError(message);
    },
  });

  const editDepartment = async (orgId: string, departmentId: string, data: EditDepartment) => {
